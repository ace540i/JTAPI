...
public class TSTestService extends ProviderListenerAdapter {
	...
	public String[] getServices() throws Exception {
		try {
			...
			final JtapiPeer jtapiPeer = JtapiPeerFactory
					.getJtapiPeer(isNull(data.getPeer()) ? null : data
							.getPeer());
			...
			trace.append("Provider could not be created.\n Please verify "
					+ "that parameters " + "in the page are correct.\n\n");
			throw e;
		}
	}
	...
	public void login() throws Exception {
		try {
			String providerString = data.getService() + ";loginID="
					+ data.getLogin() + ";passwd=" + data.getPassword();
			// create JtapiPeer
			JtapiPeer jtapiPeer = JtapiPeerFactory.getJtapiPeer(isNull(data
					.getPeer()) ? null : data.getPeer());
	...
	public void makeCall() {
		...
		if (provider != null) {
			...
			System.out
					.println("Unable to make call - provider was not created "
							+ "successfully.\n\n");
	...
	public void hangup() {
		if ((terminal.getTerminalConnections()) != null) {
			TerminalConnection terminalConnection = terminal
